...
class spider():
    # 启动时打印提示信息，self参数是必须的，去掉的话编译会报错
    def __init__(self):
        print('爬虫启动.......')
    ...
    def getmyideaInfo(self, source):
        li = re.findall(
            '<li class=" j_thread_list clearfix"(.*?)</li>', source, re.S)
        return li

    # """findall()返回的是括号所匹配到的结果（如regex1），多个括号就会返回多个括号分别匹配到的结果，
    # 如果没有括号就返回就返回整条语句所匹配到的结果.只返回匹配的内容，不包括前后的描述条件"""
    ...
    def getinfo(self, classinfo):
        ...
        info['主题'] = re.findall(
            'target="_blank" class="j_th_tit ">(.*?)</a>', classinfo, re.S)
        info['主题作者'] = re.findall('title="主题作者:(.*?)"', classinfo, re.S)
        info['内容'] = re.findall(
            '<div class="threadlist_abs threadlist_abs_onlyline ">(.*?)</div>', classinfo, re.S)
        info['最后回复人'] = re.findall(
            ' <span class="tb_icon_author_rely j_replyer" title="最后回复人:(.*?)">', classinfo, re.S)
        return info
...
if __name__ == '__main__':  # 主程序的入口
    classInfo: List[Dict[str, List[Any]]] = []  #这个列表用来存储什么的呢？每一个记录都是一篇帖子
    ...
    mySpider = spider()
    netPage = mySpider.ChangePage(url, 100)  # 每页50个帖子。那么这个100啥意思？
    for each in netPage:
        print('正在处理：' + each)
